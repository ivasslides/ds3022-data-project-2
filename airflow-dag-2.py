...
def monitoring(queue_url):
    # queue_url will be passed from payload['sqs_url'] from get_messages()
    received_messages{}
    count = 0
...
def arrange_messages(received_messages):
    sorted_msg = sorted(received_messages.itmes())
...
# function 4 
def send_solution(uvaid='fbv2sc', phrase, platform='airflow'):
...
with DAG(
    ...
    task_2 = PythonOperator(
        task_id = 'monitoring(queue_url)', 
        python_callable = monitoring(queue_url), 
    )

    # task 3, which is function 3
    task_3 = PythonOperator(
        task_id = 'arrange_messages(received_messages)',
        python_callable = arrange_messages(received_messages),
    )

    # task 4, which is function 4
    task_4 = PythonOperator(
        task_id = 'send_solution(uvaid, phrase, platform)',
        python_callable = send_solution(uvaid, phrase, platform), 
    )
